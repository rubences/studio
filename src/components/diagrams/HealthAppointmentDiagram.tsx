export function HealthAppointmentDiagram({ className }: { className?: string }) {
  ...
  const steps = [
    { title: "1. Petición del Paciente", description: "'Necesito cita con Dr. García por la tarde'", icon: "MessageSquare", color: "primary", y: 20 },
    { title: "2. Búsqueda y Propuesta", description: "Agente consulta Calendar API, encuentra 3 huecos y los ofrece por WhatsApp.", icon: "CalendarDays", color: "accent", y: 120 },
    { title: "3. Confirmación", description: "Paciente elige. Agente bloquea cita, y agenda recordatorio 24h antes.", icon: "Sparkles", color: "green-500", y: 220 }
  ];
         ...
         <g key={i} transform={`translate(${i % 2 === 0 ? 100 : 450}, ${step.y})`} filter="url(#shadow_health)">
            ...
            <foreignObject x="10" y="10" width="230" height="70">
                 <div xmlns="http://www.w3.org/1999/xhtml" className="flex items-center gap-4">
                     <div className={`flex-shrink-0 p-3 rounded-lg bg-${step.color}/10 text-${step.color}`}>
                     ...
                     <div>
                        <h3 className={`font-semibold text-${step.color}`}>{step.title}</h3>
