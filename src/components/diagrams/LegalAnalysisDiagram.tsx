export function LegalAnalysisDiagram({ className }: { className?: string }) {
  ...
  const steps = [
    { title: "1. Ingesta Segura", description: "Abogado sube un contrato de 50 páginas en PDF para una revisión urgente.", icon: "FileText", color: "hsl(var(--primary))" },
    { title: "2. Análisis con IA", description: "El agente procesa el documento, identifica y extrae cláusulas de riesgo (penalizaciones, exclusividad, etc.).", icon: "Scale", color: "hsl(var(--accent))" },
    { title: "3. Resumen Ejecutivo", description: "Genera un informe de 1 página resumiendo desviaciones, riesgos y sugiriendo modificaciones.", icon: "Bot", color: "hsl(var(--chart-4))" }
  ];
      ...
      {steps.map((step, i) => (
        <g key={i}>
          ...
          <g transform={`translate(${50 + i * 220}, 40)`} filter="url(#shadow_legal)">
            <rect x="20" y="20" width="180" height="180" rx="12" className={boxStyle} />
            ...
            <foreignObject x="30" y="35" width="160" height="150">
                <div xmlns="http://www.w3.org/1999/xhtml" className="flex flex-col items-center text-center">
                    <div className="p-4 rounded-full" style={{ backgroundColor: `${step.color}1A`, color: step.color }}>
                    ...
                    <h3 className="font-semibold text-foreground text-lg mt-3" style={{ color: step.color }}>{step.title}</h3>
