export function EcommerceReturnsDiagram({ className }: { className?: string }) {
  ...
  const steps = [
    { title: "1. Solicitud Cliente", description: "Cliente inicia devolución desde su área personal para el pedido #12345.", icon: "ShoppingCart", color: "hsl(var(--primary))" },
    { title: "2. Validación y Logística", description: "Agente verifica política, genera etiqueta de envío con API de mensajería y la envía por email.", icon: "PackageSearch", color: "hsl(var(--accent))" },
    { title: "3. Seguimiento y Reembolso", description: "Monitorea el envío. Al llegar a almacén, procesa el reembolso y notifica al cliente.", icon: "Truck", color: "hsl(var(--chart-4))" }
  ];
        ...
        <g key={i} transform={`translate(${50 + i * 250}, 50)`} filter="url(#shadow_ecomm)">
          ...
          <foreignObject x="20" y="30" width="160" height="140">
            <div xmlns="http://www.w3.org/1999/xhtml" className="flex flex-col items-center text-center h-full">
              <div className="p-3 rounded-full" style={{ backgroundColor: `${step.color}1A`, color: step.color }}>
              ...
              <h3 className="font-semibold text-base mt-2" style={{ color: step.color }}>{step.title}</h3>
